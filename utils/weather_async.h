...
#define ICON_POSITIONS 0,9
#define PRESSURE_POSITIONS 0,40
...
namespace Weather
{
    ...
    const uint8_t *getIconByCode(const char *code)
    {
...
#ifdef NEW_BMP
        case "01d"_h:
            return Icons::bmp_01d;
        case "01n"_h:
            return Icons::bmp_01n;
        case "02d"_h:
            return Icons::bmp_02d;
        case "02n"_h:
            return Icons::bmp_02n;
        case "03d"_h:
        case "03n"_h:
            return Icons::bmp_03d;
        case "04d"_h:
        case "04n"_h:
            return Icons::bmp_04d;
        case "09d"_h:
        case "09n"_h:
            return Icons::bmp_09d;
        case "10d"_h:
            return Icons::bmp_10d;
        case "10n"_h:
            return Icons::bmp_10n;
        case "11d"_h:
        case "11n"_h:
            return Icons::bmp_11d;
        case "13d"_h:
        case "13n"_h:
            return Icons::bmp_13d;
        case "50d"_h:
        case "50n"_h:
            return Icons::bmp_50d;
#else
        case "01d"_h:
            return Icons::icon_0_1;
        case "01n"_h:
            return Icons::icon_2_2;
        case "02d"_h:
            return Icons::icon_0_2;
        case "02n"_h:
            return Icons::icon_0_3;
        case "03d"_h:
            return Icons::icon_0_0;
        case "03n"_h:
            return Icons::icon_0_0;
        case "04d"_h:
            return Icons::icon_0_0;
        case "04n"_h:
            return Icons::icon_0_0;
        case "09d"_h:
            return Icons::icon_1_1;
        case "09n"_h:
            return Icons::icon_1_1;9
        case "10d"_h:
            return Icons::icon_0_4;
        case "10n"_h:
            return Icons::icon_1_4;
        case "11d"_h:
            return Icons::icon_1_0;
        case "11n"_h:
            return Icons::icon_1_0;
        case "13d"_h:
            return Icons::icon_1_2;
        case "13n"_h:
            return Icons::icon_1_2;
        case "50d"_h:
            return Icons::icon_2_1;
        case "50n"_h:
            return Icons::icon_2_1;
#endif
        }
    ...
    const bool wrongUpdateCount(const WrongUpdate wu = GET)
    {
        static uint8_t _wrongUpdateCount = 0;
        switch (wu)
        {
        case RESET:
            _wrongUpdateCount = 0;
            pointStop(0,"Reset wrong update count\n");
            break;
        case INCREASE:
            _wrongUpdateCount++;
            pointStop(0,"Increase wrong update count %u\n", _wrongUpdateCount);
            break;
        case USE_FAST_RECONNECT:
            return _wrongUpdateCount < 3;
        case NEED_TO_RESTART:
            //_wrongUpdateCount ++;
            return _wrongUpdateCount > 9;
        }
        return _wrongUpdateCount;
    };
    // const uint8_t wrongUpdateCount(const WrongUpdate wu = GET)
    // {
    //     static uint8_t _wrongUpdateCount = 0;
    //     switch (wu)
    //     {
    ...
    void onRequestComplete()
    {
        if (httpsClient.getStatusCode() == 200)
        {
            ...
            //
            //pointStop(0, "Header Date: '%s'\n",httpsClient.getDate().c_str());
            //time_t requestTime = parseGMTString(httpsClient.getHeader("Date").c_str());
            ...
            if (updated.code() == DeserializationError::Ok)
            {
                ...
                #ifdef USE_WEATHER_SERVER_TIME
                time_t responseTime = httpsClient.getServerResponseTime();
                if ( responseTime ) {
                    pointStop(0, "Set time from server '%s' (+%lums)\n", 
                        httpsClient.getDate().c_str(), httpsClient.getResponseDelay()
                    );
                    setTime( responseTime ); 
                }
                #endif
                String _city = doc["name"].as<String>();
                if (getLang(GeoLocationAsync::myLocation).isEmpty() || Translit::check( _city )) {
                    strcpy(Weather::data.cityName, utf8rus(Translit::toCyrillic( _city)).c_str());
                } else { 
    ...
    String getLang(GeoLocationAsync::GeoData &location)
    {
        ...
        code.toLowerCase();
        if (code.equals("russia") || code.equals("ru"))
    ...
    {
        if (!WiFi.isConnected())
            return AsyncRequest::NoConnection;
        ...
        {
            requestUri += "lat=";
            requestUri += String(GeoLocationAsync::myLocation.latitude, 4);
            requestUri += "&lon=";
            requestUri += String(GeoLocationAsync::myLocation.longitude, 4);
    ...
    void drawIcon(Adafruit_PCD8544 &display, const uint8_t *bitmap, const uint8_t width = 32, const uint8_t height = 32)
    {
        ...
        if (bitmap)
            display.drawBitmap(ICON_POSITIONS, bitmap, width, height, PRINT_COLOR);
    }
    ...
    void printData(Adafruit_PCD8544 &display, const Data &data, bool show = false)
    {
        ...
        display.setTextSize(1);
        display.setCursor(PRESSURE_POSITIONS);
        display.print(data.pressure);
        display.print("mmHg");

        //str = String(''); //String(data.tempFeel, 0);
    ...
    void updateDataDS(Adafruit_PCD8544 &display)
    {
        ...
        case AsyncRequest::WaitWiFiConnection:
            ...
            if (WiFi.isConnected())
            {
                ...
                {
                    updateState = AsyncRequest::RespondWaiting;
                    //wrongUpdateCount(RESET);
                }
                else
                {
                    updateState = AsyncRequest::FailRespond;
                    if ( wrongUpdateCount(NEED_TO_RESTART)) 
                        ESP.restart();
                    // if (wrongUpdateCount(INCREASE) >= 10)
                    //     ESP.restart();
                }
            }
        ...
        if (weatherTick.refresh())
        {
            ...
            pointStop(0, "Free=%u, Max block=%u\n",
                ESP.getFreeHeap(),
    ...
    void handleTick()
    {
        ...
        if (weatherTick.tick())
        {
            if (updateState == AsyncRequest::Idle)
            {
                ...
                {
                    updateState = AsyncRequest::WaitWiFiConnection;
                    Reconnect::connect(0, wrongUpdateCount(USE_FAST_RECONNECT)); // wrongUpdateCount() < 3);
                }
            }
            // httpsClient.update();
        }
    ...
    void test()
    {
        ...
        if (!httpsClient.isBusy())
        {
            ...
            httpsClient.get(url, [](int statusCode, const String &headers)
                            {
                    // Обработка заголовков (если нужно)
                    Serial.println( statusCode); }, [](const String &chunk)
                            {
                                // Обработка данных по мере поступления (если нужно)
                            },
                            onRequestComplete, [](const String &error)
                            {
